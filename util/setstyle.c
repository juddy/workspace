/* setstyle.c - loads style related options to workspace
 *
...
static void print_help(int print_usage, int exitval)
{
	...
	if (print_usage) {
		puts("Reads style/theme configuration from FILE and updates Workspace.");
		puts("");
...
int main(int argc, char **argv)
{
	...
	if (!prop) {
		perror(path);
		printf("%s: could not load Workspace configuration file.\n", __progname);
		return 1;
	}
