...
int main(int argc, char *argv[])
{
	 ...
	 *       L3Menu               Solid
	 *         L4Menu               Indigo
	 *   L1Menu               Configure Workspace
	 *
	 */

	/* Root */
	RMenu = WMCreatePLArray(WMCreatePLString("Workspace"), NULL);
	...
	find_and_write("Workspace", Workspace, 0);
...
#define SOLID_BACK(label, colorspec)									\
	L4Menu = WMCreatePLArray(									\
		...
		WMCreatePLString("wdwrite Workspace WorkspaceBack '(solid, \"" colorspec "\")'"),	\
		NULL											\
	);												\
...
#define GRADIENT_BACK(label, fcolorspec, tcolorspec)							\
	L4Menu = WMCreatePLArray(									\
		WMCreatePLString(label),								\
		WMCreatePLString("EXEC"),								\
		WMCreatePLString("wdwrite Workspace WorkspaceBack '(vgradient, \"" 			\
	...
	L4Menu = WMCreatePLArray(
		...
		WMCreatePLString("EXEC"),
		WMCreatePLString("wdwrite Workspace WorkspaceBack "
...
#undef GRADIENT_BACK
	...
	/* Root -> Configure Workspace */
	L1Menu = WMCreatePLArray(
		WMCreatePLString(_("Configure Workspace")),
		WMCreatePLString("EXEC"),
	...
	/* Root -> Restart Workspace */
	L1Menu = WMCreatePLArray(
		WMCreatePLString(_("Restart Workspace")),
	...
	/* Root -> Exit Workspace */
	L1Menu = WMCreatePLArray(
		WMCreatePLString(_("Exit Workspace")),
