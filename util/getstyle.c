/* getstyle.c - outputs style related options from Workspace to stdout
 *
...
static void print_help(int print_usage, int exitval)
{
	...
	if (print_usage) {
		puts("Retrieves style/theme configuration and outputs to ~/.workspace/Library/Workspace/Themes/file.themed/style or to stdout");
		puts("");
...
static void makeThemePack(WMPropList * style, const char *themeName)
{
	...
	snprintf(themeDir, themeNameLen, "%s/Library/Workspace/Themes/%s.themed/", t, themeName);
	ThemePath = themeDir;
...
int main(int argc, char **argv)
{
	...
	if (!prop) {
		printf("%s: could not load Workspace configuration file \"%s\".\n", __progname, path);
		return 1;
	}

	/* get global value */
	path = wglobaldefaultspathfordomain("Workspace");
