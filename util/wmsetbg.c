 ...
 * 		workspace background setting helper for workspace
 *
...
static void helperLoop(RContext * rc)
{
	...
	while (1) {
		...
		if (readmsg(0, buffer, 4) < 0) {
			werror("error reading message from Workspace");
			errcount--;
			if (errcount == 0) {
				wfatal("quitting");
				exit(1);
...
static void print_help(void)
{
	...
	puts("optionally update Workspace configuration");
	puts("");
	...
	puts(" -u, --update-workspace              update Workspace domain database");
...
static void changeTextureForWorkspace(const char *domain, char *texture, int workspace)
{
	...
	array = getValueForKey("Workspace", "WorkspaceSpecificBack");
...
int main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-helper") == 0) {
			...
			xineStretch = True;
#endif
		} else if (strcmp(argv[i], "-u") == 0 || strcmp(argv[i], "--update-workspace") == 0) {
			update++;
