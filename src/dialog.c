...
static char *HistoryFileName(const char *name)
{
	...
	filename = wstrappend(filename, "/.AppInfo/Workspace/History");
	if (name && strlen(name)) {
...
void wShowInfoPanel(WScreen * scr)
{
	...
	WMResizeWidget(panel->win, 390, 430);
...
void wShowLegalPanel(WScreen * scr)
{
	...
	WMSetLabelText(panel->licenseL,
		       _("    Workspace is free software; you can redistribute it and/or\n"
...
static WMPixmap *getWorkspaceIconImage(WMScreen *scr)
{
	WMPixmap *pix = NULL;
	char *path = NULL;
...
int wShowCrashingDialogPanel(int whatSig)
{
	...
	logo = getWorkspaceIconImage(scr);
	if (logo) {
	...
	snprintf(buf, sizeof(buf), _("Workspace received signal %i."), whatSig);
	...
	WMSetLabelText(panel->note2L,
			 ...
			 " Please fill the included BUGFORM and " "report it to bugs@workspace.info."));
	...
	WMAddPopUpButtonItem(panel->whatP, _("Restart Workspace"));
