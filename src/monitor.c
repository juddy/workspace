/* monitor.c - monitors the workspace process
 *
...
int MonitorLoop(int argc, char **argv)
{
	...
	for (;;) {
		...
		if (pid == 0) {
			...
			werror(_("Error respawning Workspace"));
			exit(1);
		} else if (pid < 0) {
			werror(_("Error respawning Workspace"));
			exit(1);
		...
		do {
			if ((exited = waitpid(-1, &status, 0)) < 0) {
				werror(_("Error during monitoring of Workspace process."));
				error = True;
				break;
			}
		} while (exited != pid);
		...
		/* Check if the workspace process exited due to a crash */
		if (WIFSIGNALED(status) &&
			...
			wwarning(_("Workspace exited due to a crash (signal %i) and will be restarted."),
				 WTERMSIG(status));
		} else
			break;
	}
