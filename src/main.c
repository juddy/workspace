...
void Restart(char *manager, Bool abortOnFailure)
{
	...
	if (!prog) {
		...
		wfatal(_("failed to restart Workspace."));
	} else {
		execvp(prog, argv);
...
static void print_help(void)
{
	...
	puts(_("The Workspace window manager for the X window system"));
	puts("");
...
static void execInitScript(void)
{
	...
	paths = wstrconcat(wusergnusteppath(), "/Library/Workspace");
...
int main(int argc, char **argv)
{
	...
	/* setup common stuff for the monitor and workspace itself */
	WMInitializeApplication("Workspace", &argc, argv);
...
static int real_main(int argc, char **argv)
{
	...
	/* for telling WPrefs what's the name of the workspace binary being ran */
	...
	/* add the extra option to signal that we're just restarting workspace */
