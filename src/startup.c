...
void StartUp(Bool defaultScreenOnly)
{
	 ...
	 * who exec()-ed us. This can happen for example if Workspace crashes
	...
	WDWorkspace = wDefaultsInitDomain("Workspace", True);
	if (!WDWorkspace->dictionary)
		wwarning(_("could not read domain \"%s\" from defaults database"), "Workspace");

	/* read defaults that don't change until a restart and are
	 * screen independent */
	wReadStaticDefaults(WDWorkspace ? WDWorkspace->dictionary : NULL);
