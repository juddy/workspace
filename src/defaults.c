...
#define GLOBAL_DEFAULTS_SUBDIR "Workspace"
#endif

/***** Global *****/
extern WDDomain *WDWorkspace;
...
WDefaultEntry staticOptionList[] = {
	...
	{"DisableClip", "YES", (void *)WM_CLIP,
	    NULL, getBool, setIfDockPresent, NULL, NULL},
	{"DisableDrawers", "NO", (void *)WM_DRAWER,
	    NULL, getBool, setIfDockPresent, NULL, NULL},
	{"ClipMergedInDock", "YES", NULL,
	    NULL, getBool, setClipMergedInDock, NULL, NULL},
	{"DisableMiniwindows", "YES", NULL,
...
WDefaultEntry optionList[] = {
	...
	{"RaiseAppIconsWhenBouncing", "YES", NULL,
	    &wPreferences.raise_appicons_when_bouncing, getBool, NULL, NULL, NULL},
	{"DoNotMakeAppIconsBounce", "YES", NULL,
	...
	{"TitleJustify", "left", seJustifications,
...
void wDefaultsCheckDomains(void* arg)
{
	...
	if (stat(WDWorkspace->path, &stbuf) >= 0 && WDWorkspace->timestamp < stbuf.st_mtime) {
		WDWorkspace->timestamp = stbuf.st_mtime;

		/* Global dictionary */
		shared_dict = readGlobalDomain("Workspace", True);

		/* User dictionary */
		dict = WMReadPropListFromFile(WDWorkspace->path);
		...
		if (dict) {
			if (!WMIsPLDictionary(dict)) {
				...
				if (WDWorkspace->dictionary)
					WMReleasePropList(WDWorkspace->dictionary);

				WDWorkspace->dictionary = dict;
			}
		} else {
			wwarning(_("could not load domain %s from user defaults database"), "Workspace");
		}
...
void wReadDefaults(WScreen * scr, WMPropList * new_dict)
{
	...
	void *tdata;
	WMPropList *old_dict = (WDWorkspace->dictionary != new_dict ? WDWorkspace->dictionary : NULL);
...
getWSSpecificBackground(WScreen * scr, WDefaultEntry * entry, WMPropList * value, void *addr, void **ret)
{
 ...
 again:
	...
	*ret = WMRetainPropList(value);
	 ...
	 * If the WorkspaceSpecificBack is changed once workspace has started,
	...
	if (!scr->flags.backimage_helper_launched && !scr->flags.startup) {
		...
		WMRemoveFromPLDictionary(WDWorkspace->dictionary, key);
