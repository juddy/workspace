...
BUILDLOG="${HOME}/log/workspace-testing-nightly.log"
...
if [ -n "${WMAKER_DISTRO}" ]; then
  case ${WMAKER_DISTRO} in
    unstable)
      BUILDLOG="${HOME}/log/workspace-unstable-nightly.log"
      ;;
  esac
fi
...
doTarball() {
	echo tar -czf ../workspace_${BASEVERSION}-${REPOVERSION}.orig.tar.gz .
	tar -czf ../workspace_${BASEVERSION}-${REPOVERSION}.orig.tar.gz .
}
