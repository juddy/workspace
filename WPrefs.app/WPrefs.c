...
static void createMainWindow(WMScreen * scr)
{
	...
	WMSetLabelText(WPrefs.nameL, _("Workspace Preferences"));
	WMReleaseFont(font);
...
void Initialize(WMScreen * scr)
{
	...
	WMSetLabelText(WPrefs.statusL, _("Loading Workspace configuration files..."));
	XFlush(WMScreenDisplay(scr));
...
static void loadConfigurations(WMScreen * scr, WMWindow * mainw)
{
	...
	if (db) {
		if (!WMIsPLDictionary(db)) {
			...
			db = NULL;
			sprintf(mbuf, _("Workspace domain (%s) is corrupted!"), path);
			WMRunAlertPanel(scr, mainw, _("Error"), mbuf, _("OK"), NULL, NULL);
		}
	} else {
		sprintf(mbuf, _("Could not load Workspace domain (%s) from defaults database."), path);
		WMRunAlertPanel(scr, mainw, _("Error"), mbuf, _("OK"), NULL, NULL);
	}
	...
	if (!file || !fgets(buffer, 1023, file)) {
		...
		WMRunAlertPanel(scr, mainw, _("Error"),
				_
				("Could not extract version from Workspace. Make sure it is correctly installed and is in your PATH environment variable."),
				_("OK"), NULL, NULL);
		exit(1);
	}
	if (file)
	...
	if (v1 == 0 && (v2 < 0 || v3 < 4)) {
		sprintf(mbuf, _("WPrefs only supports Workspace 0.0.4 or newer.\n"
	...
	if (v1 > 1 || (v1 == 1 && (v2 > 0))) {
		sprintf(mbuf,
			_
			("Workspace %i.%i.%i, which is installed in your system, is not fully supported by this version of WPrefs."),
			v1, v2, v3);
		WMRunAlertPanel(scr, mainw, _("Warning"), mbuf, _("OK"), NULL, NULL);
	}
	...
	if (gdb) {
		if (!WMIsPLDictionary(gdb)) {
			...
			gdb = NULL;
			sprintf(mbuf, _("Workspace domain (%s) is corrupted!"), buffer);
			WMRunAlertPanel(scr, mainw, _("Error"), mbuf, _("OK"), NULL, NULL);
		}
	} else {
		sprintf(mbuf, _("Could not load global Workspace domain (%s)."), buffer);
		WMRunAlertPanel(scr, mainw, _("Error"), mbuf, _("OK"), NULL, NULL);
	}
